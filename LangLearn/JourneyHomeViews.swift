...
struct JourneySetupView: View {
    ...
    @State private var profile = JourneyProfile()
    ...
    var body: some View {
        Form {
            Section {
                MiloAvatarView(mood: .still, size: 120).frame(maxWidth: .infinity)
                Text("Din resa börjar där du är.").font(.largeTitle.bold())
            ...
            Section("Din startpunkt") {
                ...
                Text("Att känna igen tecknen och att kunna prata är olika saker. Milo hjälper dig med båda.")
                    .font(.footnote).foregroundStyle(.secondary)
            }
            Section("Något att längta till") {
                TextField("Jag vill kunna…", text: $profile.goal, axis: .vertical).lineLimit(2...4)
                Text("Till exempel: beställa lunch, prata med min partner eller förstå en låt.").font(.footnote).foregroundStyle(.secondary)
                TextField("Saker jag gillar (valfritt)", text: $profile.interests, axis: .vertical).lineLimit(1...3)
                Picker("En lagom stund", selection: $profile.minutes) {
                    ForEach([3, 5, 10], id: \.self) { Text("\($0) min").tag($0) }
                }.pickerStyle(.segmented)
            }
            Section {
                Text("Dina val och dina övningsresultat används för kommande uppgifter. Du kan se och ändra profilen i Min resa. När du skapar en AI-lektion skickas relevant studiekontext till OpenAI.")
                    .font(.footnote).foregroundStyle(.secondary)
                if let error { Text(error).foregroundStyle(.red) }
                Button(editing ? "Spara mina val" : "Visa min start") {
                    do {
                        try profile.validate()
                        try store.updateJourney { $0.profile = profile }
                        dismiss()
