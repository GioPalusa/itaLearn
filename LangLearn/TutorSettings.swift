...
final class TutorSettings {
    ...
    private enum Key {
        ...
        static let targetLanguage = "tutor.targetLanguage"
        static let nativeLanguage = "tutor.nativeLanguage"
        static let hasChosenLanguage = "tutor.hasChosenLanguage"
    }
    ...
    var initials: String {
        let parts = learnerName
            ...
            .prefix(2)
            .compactMap(\.first)
        return parts.isEmpty ? targetLanguage.code.uppercased() : parts.map { String($0).uppercased() }.joined()
    }
