...
private func pack() -> PracticePack {
    PracticePack(flashcards: (0..<4).map { Flashcard(id: "card-\($0)", cue: "En kaffe", answer: "Un caffè", example: "Vorrei un caffè.") },
                 puzzles: (0..<3).map { SentencePuzzle(id: "puzzle-\($0)", cue: "Jag vill ha en kaffe", answers: [["Vorrei", "un", "caffè."]], words: ["caffè.", "Vorrei", "un", "una"], explanation: "Caffè är maskulint: un caffè.") })
}
...
private actor ActivityService: LearningService {
    ...
    func teach(_ context: LessonContext) async throws -> LessonReply {
        teaching += 1
        return LessonReply(reply: "Bene.", translation: "Bra.", correction: nil, requiresRetry: false, retryPrompt: "", objectiveIDsAchieved: [], lessonComplete: false, memory: "Övade verb")
    }
