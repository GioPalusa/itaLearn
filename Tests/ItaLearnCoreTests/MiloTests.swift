...
struct MiloTests {
    ...
    @Test func clipHeaderSamplingAndRootOwnershipAreValid() throws {
        let library = try MiloClipLibrary(contentsOf: root.appendingPathComponent("ItaLearn/Resources/MiloClips.bin"))
    ...
    @Test func manifestMatchesSnowRuntimeContract() throws {
        let data = try Data(contentsOf: root.appendingPathComponent("ItaLearn/Resources/MiloRigManifest.json"))
        ...
        #expect(!FileManager.default.fileExists(atPath: root.appendingPathComponent("ItaLearn/Resources/clips.json").path))
    }

    @MainActor @Test func bundledModelContainsTheManifestRigAndFace() async throws {
        let entity = try await Entity(contentsOf: root.appendingPathComponent("ItaLearn/Resources/Milo.usdz"))
