...
nonisolated struct OpenAILearningService: LearningService {
    ...
    func hint(_ context: PuzzleHintContext) async throws -> PuzzleHint {
        let result = try await client.respond(
            ...
            The learner is stuck building one Italian sentence from a fixed word bank and has
            ...
            encouragement: one warm Swedish sentence naming something their attempt already got
            right. If nothing is right yet, be kind and matter-of-fact instead of inventing praise.
            hint: one or two Swedish sentences explaining the single most useful thing to fix, in
            ...
            All output is plain Swedish text, not Markdown. Keep encouragement under 400 and hint
    ...
    func morePractice(_ context: PracticeExtensionContext) async throws -> PracticePack {
        let result = try await client.respond(
            ...
            unique id, Swedish cue, Italian answer and short Italian example; and 3–8 sentence
            puzzles with a unique id, a Swedish sentence to translate, answers (1–4 valid Italian
            word orders as token arrays), a words bank of 3–16 individual Italian tokens, and a
            short Swedish explanation. Each answer has 2–14 tokens and must be buildable from the
            ...
            Keep apostrophe words like l'ingresso together, attached punctuation with its word, no
            whitespace inside a token. The app shuffles the bank locally. You may add 1–2 plausible
    ...
    func planDirections(_ context: PlanExtensionContext) async throws -> PlanDirections {
        let result = try await client.respond(
            ...
            Propose 3 to 5 concrete ways forward and recommend one. Write every field in Swedish.

            Each option is a real-life area to practise in, named the way a person would say it:
            fortsätta med resor, mat och restaurang, vänner som hälsar på, boka hotell, prata med en
            förälder, säga till barnen, festa och umgås, jobb och möten, hälsa och vardagsbestyr.
    ...
    func nextLessons(_ context: PlanExtensionContext) async throws -> PlanExtension {
        let result = try await client.respond(
            ...
            Use stable slug ids that do not appear in existingLessons, Swedish titles (max 150 chars),
            concise Swedish summaries, 1 to 5 measurable objectives, 1 to 5 successCriteria, up to 15
            Italian vocabulary items, an Italian roleplay scenario, and a realistic 3–60 minute
