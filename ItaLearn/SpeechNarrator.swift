...
/// Uses the same local Italian voice for audio and mouth animation.
...
final class SpeechNarrator: NSObject, AVAudioPlayerDelegate {
    ...
    private static var italianVoice: AVSpeechSynthesisVoice? {
        ...
        return voices.first { $0.quality == .premium }
            ...
            ?? voices.first ?? AVSpeechSynthesisVoice(language: "it-IT")
    }
...
nonisolated final class SpeechAudioWriter: @unchecked Sendable {
    ...
    private let url = FileManager.default.temporaryDirectory.appendingPathComponent("italearn-speech-\(UUID()).caf")
