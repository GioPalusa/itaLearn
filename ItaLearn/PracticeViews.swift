...
struct LessonPracticeView: View {
    ...
    private func sentenceSubtitle(_ progress: PracticeProgress) -> String {
        guard progress.hasUnfinishedSentence, let cursor = progress.puzzleCursor else {
            return "Från svenska till italienska, ord för ord"
        }
...
struct FlashcardPracticeView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let errorMessage {
                Text(errorMessage).font(.il(13)).foregroundStyle(ItaLearn.red).padding(.horizontal, 16)
            }
        }
    ...
    @ViewBuilder private var swipeBadge: some View {
        ...
        if magnitude > 0.15 {
            Text(dragOffset.width < 0 ? "ÖVA IGEN" : "KUNDE")
                ...
                .background(dragOffset.width < 0 ? ItaLearn.magenta : ItaLearn.green, in: .capsule)
                .padding(18)
                .opacity(magnitude)
        }
    }
    ...
    private func faceCard(_ card: Flashcard) -> some View {
        Button {
            withAnimation(reduceMotion ? nil : ItaLearnMotion.move) { revealed.toggle() }
        } label: {
            VStack(spacing: 0) {
                HStack {
                    ...
                    Spacer(minLength: 0)
                    if revealed {
                        Button { narrator.stop(); narrator.speak(card.italian) } label: {
                            HStack(spacing: 6) {
                ...
                VStack(spacing: 12) {
                    Text(revealed ? card.italian : card.swedish)
                ...
                Rectangle().fill(ItaLearn.hairline).frame(height: 1)
            ...
            .background(ItaLearn.card, in: .rect(cornerRadius: 22))
            .overlay { RoundedRectangle(cornerRadius: 22).strokeBorder(ItaLearn.cardBorder, lineWidth: 1) }
        ...
        VStack(spacing: 10) {
            HStack(spacing: 10) {
                Button { rate(card, known: false) } label: {
                    ...
                    .overlay { Capsule().strokeBorder(ItaLearn.purple.opacity(0.24), lineWidth: 1) }
                }
                .buttonStyle(.plain)
    ...
    @ViewBuilder private var finished: some View {
        VStack(spacing: 16) {
            Label("Rundan är klar!", systemImage: "checkmark.circle.fill")
                .font(.il(28, .bold)).foregroundStyle(ItaLearn.deepGreen)
            Text("Dina markeringar är sparade. Repetera gärna de ord du vill befästa.")
                .multilineTextAlignment(.center).foregroundStyle(ItaLearn.inkSecondary)
    ...
    private func rate(_ card: Flashcard, known: Bool) {
        do {
            ...
            withAnimation(reduceMotion ? nil : ItaLearnMotion.move) {
...
struct SentencePracticeView: View {
    ...
    var body: some View {
        ...
        .italearnCanvas()
        .hideNavigationBar()
        .motion(ItaLearnMotion.settle, checked)
        .motion(ItaLearnMotion.settle, correct)
        .onAppear(perform: restoreIfNeeded)
    ...
    @ViewBuilder private func round(_ puzzle: SentencePuzzle) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            Text("SÄG DET PÅ ITALIENSKA")
                .font(.il(11, .semibold)).tracking(0.88).foregroundStyle(ItaLearn.magenta)
            Text(puzzle.swedish)
                .font(.il(24, .bold)).foregroundStyle(ItaLearn.ink)
                .padding(.top, 8)
        ...
        if let error = errorMessage ?? chat.errorMessage {
            Text(error).font(.il(13)).foregroundStyle(ItaLearn.red).padding(.top, 12)
        }
        ...
        VStack(spacing: 12) {
            if correct {
                ...
                Button("Börja om meningen") { move { assembly.reset() } }
                    .font(.il(16, .semibold)).foregroundStyle(ItaLearn.purple)
    ...
    private func slotArea(_ puzzle: SentencePuzzle) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            Text("Dra ett ord till en plats, eller tryck för att lägga sist.")
                .font(.il(13)).foregroundStyle(ItaLearn.inkTertiary)
                .padding(.top, 10)
        }
    }
    ...
    private func filledSlot(_ tile: Int, at position: Int, puzzle: SentencePuzzle) -> some View {
        ...
        .overlay(alignment: .leading) {
            if targetedSlot == position {
                Capsule().fill(ItaLearn.magenta).frame(width: 4, height: 40).offset(x: -5)
            }
        }
    ...
    private func emptySlot(at position: Int, puzzle: SentencePuzzle, isNext: Bool) -> some View {
        ...
        return RoundedRectangle(cornerRadius: 12)
            .strokeBorder(
                highlighted ? ItaLearn.purple.opacity(targeted ? 0.9 : 0.4) : Color.black.opacity(0.16),
                style: StrokeStyle(lineWidth: 2, dash: [6, 4])
            )
            .background(
                highlighted ? ItaLearn.purple.opacity(targeted ? 0.14 : 0.06) : .clear,
    ...
    private func placedTile(_ word: String) -> some View {
        Text(word)
            ...
            .background(ItaLearn.purple, in: .rect(cornerRadius: 12))
            .shadow(color: ItaLearn.purple.opacity(0.22), radius: 4, y: 4)
    }
    ...
    private func bankTile(_ tile: Int, puzzle: SentencePuzzle, used: Bool, highlighted: Bool) -> some View {
        if used {
            ...
            Button {
                ...
                Text(puzzle.words[tile])
                    .font(.il(18, .semibold)).foregroundStyle(ItaLearn.purple)
                    .padding(.horizontal, 15).frame(height: 44)
                    .background(ItaLearn.card, in: .rect(cornerRadius: 12))
                    .overlay {
                        RoundedRectangle(cornerRadius: 12)
                            .strokeBorder(highlighted ? ItaLearn.magenta : ItaLearn.cardBorder,
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if correct {
                Button("Lyssna", systemImage: "speaker.wave.2") {
                    narrator.stop(); narrator.speak(assembly.words(in: puzzle).joined(separator: " "))
                }
            } else {
    ...
    private func hintCard(_ hint: PuzzleHint) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: "sparkles")
                    .font(.system(size: 13)).foregroundStyle(ItaLearn.purple).padding(.top, 2)
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    if !hint.nextWord.isEmpty {
                        Text("Prova \(hint.nextWord) härnäst — ordet är markerat nedan.")
                            .font(.il(13, .semibold)).foregroundStyle(ItaLearn.magenta)
                    }
                }
            }
        }
        ...
        .overlay { RoundedRectangle(cornerRadius: 14).strokeBorder(ItaLearn.cardBorder, lineWidth: 1) }
    }
    ...
    @ViewBuilder private var finished: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Button("Spela igen från början") { index = 0; reset() }.buttonStyle(ItaLearnSecondaryButtonStyle())
            if let error = chat.errorMessage { Text(error).foregroundStyle(ItaLearn.red) }
        }
    }
    ...
    private func move(_ change: () -> Void) {
        withAnimation(reduceMotion ? nil : ItaLearnMotion.move) { change() }
        checked = false
        persistPosition()
    }
    ...
    private func advance() {
        ...
        withAnimation(reduceMotion ? nil : ItaLearnMotion.move) { index += 1 }
